...
def test_two_party_share_ignores_independent_votes():
    rows = _race(40, 50, 10)
    assert abs(two_party_dpp_share(rows) - 50/90) < 1e-12
    assert abs(major_party_coverage(rows) - .9) < 1e-12


def test_information_weight_downweights_independent_dominance():
    normal = information_weight(_race(48, 49, 3))
    factional = information_weight(_race(11, 31, 58))
    assert normal > factional


def test_temporal_leakage_gate_rejects_same_day_information():
    try:
        assert_available_before("2022_council", "2022-11-26", "2022-11-26")
    except FutureDataLeakage:
        pass
    else:
        raise AssertionError("same-day feature should have been rejected")


def test_weighted_ridge_predicts_valid_two_party_shares():
