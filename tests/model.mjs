...
assert.notDeepEqual(before.mean,after.mean);
assert.equal(after.sd.length,before.sd.length);
assert.ok(after.sd.every(Number.isFinite));
assert.ok(after.sd.every(x=>x>0));
