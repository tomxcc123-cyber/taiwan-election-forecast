...
const before=posterior(data.counties[0],feed,data),after=posterior(data.counties[0],mock,data);
assert.notDeepEqual(before.mean,after.mean);assert.ok(after.sd[0]<before.sd[0]);
assert.notDeepEqual(simulate(data,mock).counties[0].shares,r.counties[0].shares);
