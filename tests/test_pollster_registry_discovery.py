...
class PollsterRegistryDiscoveryTests(unittest.TestCase):
    ...
    def test_registry_has_unique_governed_pollsters(self):
        ...
        self.assertIn('2028總統', self.pollsters['shanshui']['discovery']['exclude_cues'])
        self.assertIn('誰較有利', self.pollsters['shanshui']['discovery']['exclude_cues'])
    ...
    def test_zmedia_discovery_finds_mayoral_pages_not_issue_noise(self):
        ...
        raw = '''<html><head><meta charset="utf-8"></head><body>
          ...
          <a href="/Document/Detail/50003">震撼民心 震傳媒民調（一）／2028國民黨四大天王大洗盤；2028總統大選20.4%支持蔣萬安</a>
          <a href="/Document/Detail/50004">震傳媒民調（三）／蔣若連任北市長續戰2028？針對2028總統大選是否支持蔣萬安參選</a>
          <a href="/Document/Detail/50005">震傳媒民調（五）／事件未見外溢效應！對高雄市長選情無影響；若持續討論對賴瑞隆或柯志恩誰較有利</a>
    ...
    def test_merge_discovery_queues_unknown_and_never_changes_records(self):
        ...
        pages = {
            ...
            shanshui['discovery']['index_urls'][1]: '''<html><head><meta charset="utf-8"></head><body>
              <a href="/Document/Detail/50001">震傳媒民調（二）／55.2%北市民支持蔣萬安連任 45.3%綠支持者不看好沈伯洋</a>
              <a href="/Document/Detail/50002">震傳媒民調（一）／65.7%北市民滿意蔣萬安施政 近兩成五綠營支持者亦肯定</a>
              <a href="/Document/Detail/50004">震傳媒民調（三）／蔣若連任北市長續戰2028？針對2028總統大選是否支持蔣萬安參選</a>
              <a href="/Document/Detail/50005">震傳媒民調（五）／事件未見外溢效應！對高雄市長選情無影響；若持續討論對賴瑞隆或柯志恩誰較有利</a>
        ...
        for noise_id in ('50002', '50004', '50005'):
            self.assertNotIn(f'https://www.zmedia.com.tw/Document/Detail/{noise_id}', queued)
