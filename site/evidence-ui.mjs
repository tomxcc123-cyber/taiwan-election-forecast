...
export function candidateEvidence(data,r){
  ...
  ${events.map(e=>`<article class="evidence-event"><span class="eyebrow">已核對公開報導 · ${esc(e.date)}</span><h3>${esc(e.title)}</h3><p>${esc(e.note)}</p><a href="${link(e.source_url)}" target="_blank" rel="noopener">${esc(e.source_name)}</a><span class="badge">效果未估計</span></article>`).join('')||'<p class="meta">尚無已核對的跨黨支持紀錄，不代表沒有支持關係。</p>'}
  <details><summary>哪些因素已進入模型？</summary><p>前屆同名個人得票、具名政黨得票、前屆勝方與缺值標記已用於基本面。履歷文字及以上支持事件只供證據閱讀，沒有轉成能力評分或額外加分。現任其他職務不等於現任縣市長。尚未估計人口分群、地方派系或投票率效果。</p><p>${esc(data.evidence.grading_rule)}</p><p>${esc(data.evidence.coverage)}</p></details></section>`;
}
...
export function presetState(data,county,presetId,selection){
  ...
  return cleanState({county,edits:{[r.race_id]:{...selection,boost:p.boost,flow:p.flow,tactical:p.tactical}},constraints:{}},data);
}
